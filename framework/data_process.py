...
def dataset_process(home_dir, dataset_name):
    ...
    elif dataset_name == 'opposed_reasoning':
        input_file = f'{home_dir}/ImplexConv_opposed.json'
        with open(input_file, 'r') as f:
            data = json.load(f)        
        conversations = [item['conversation'] for item in data]
        qa_pairs = [item['qa'] for item in data]

        return conversations, qa_pairs

    elif dataset_name == 'supportive_reasoning':
        input_file = f'{home_dir}/ImplexConv_supportive.json'
        with open(input_file, 'r') as f:
            data = json.load(f)
        conversations = [item['conversation'] for item in data]
        qa_pairs = [item['qa'] for item in data]
