...
def main(
        ...
        output_file = 'all_intent_facts.json',
    ):
    # if file exist,  load it
    conversations, qa_pairs = dataset_process(home_dir, dataset_name)
    if os.path.exists(f'{home_dir}/{output_file}'):
        with open(f'{home_dir}/{output_file}', 'r') as f:
    ...
    for i, conversation in tqdm(enumerate(conversations), desc='Processing conversations'):
        dialog_list = []
        batch_size = 20
        ...
        with open(f'{home_dir}/{output_file}', 'w') as f:
