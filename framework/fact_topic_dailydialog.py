...
def get_fact(text):
    ...
    for fact in facts:
        fact_list.append(fact.split(')')[1].strip())
    return fact_list
...
def rag_original(question, summarization_list, original_list, model, top_k=2):
    ...
    sel_sum_list = []
    for idx in top_sum:
        if sum_similarities[idx] > 0.4:
            sel_sum_list.append(summarization_list[idx])
    sel_org_list = []
    for idx in top_org:
        if org_similarities[idx] > 0.4:
...
def main(
    ...
    for index, uni_fact in enumerate(sum_fact[len(final_response):]):
        ...
        if len(speaker1_fact) > 2:
            summarized_facts1, topic_dic1 = get_clustered_facts(speaker1_fact, model, 2, model_type)
        if len(speaker2_fact) > 2:
            summarized_facts2, topic_dic2 = get_clustered_facts(speaker2_fact, model, 2, model_type)
        ...
        for qa_pair in questions[index]:
            ...
            # sel_sum_list, sel_org_list = rag_original(qa_pair['question'], summarization_list, conversations[index], model, top_k=2)
            rag_sumy_response_text = rag_sumy_prompt(cur_turn, summarization_list, qa_pair['question'])
            all_response_dic['rag_sumy'] = rag_sumy_response_text
            print(f'rag_sumy_response_text {rag_sumy_response_text}')

            rag_org_response_text = rag_original_prompt(cur_turn, conversations[index], qa_pair['question'])
