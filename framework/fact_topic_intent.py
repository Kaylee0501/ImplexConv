...
def ground_truth_prompt(question, intent_conversation, llm_model_name):
    ...
    Could that directly implys the question that {question}?
    The answer shoud only be 'YES' or 'No'.
    '''
...
class HierarchicalClusteringSummarizer:
    ...
    def find_related_facts(self, question):
        ...
        current_related = self.select_fact(question, current_summaries)
        ...
        while current_level > 0:
            ...
            for summary in current_related:
                ...
                if children:
                    related_children = self.select_fact(question, children)
...
def fact_prompt(question, speaker1_fact, llm_model_name):
    ...
    Given these traits as background information, could that directly implys the question that {question}?
    Your answer should be aligned with the personal traits of speaker1.
    Your answer shoud only be 'YES' or 'No'.
    '''
    completion = client.chat.completions.create(
        # model="gpt-4-turbo-preview",
        model=llm_model_name,
...
def long_context_prompt(question, conversation, llm_model_name):
    client = OpenAI()
    prompt = f'''
    Given the previous conversations {conversation} as background information, could that directly implys the question that {question}?
...
def rag_sumy_prompt(question, summary, llm_model_name):
    client = OpenAI()
    prompt = f'''
    Given the related summary of Speaker1 and Assistant's conversation: {summary} as background information, could that directly implys the question that {question}?
...
def rag_original_prompt(question, original, llm_model_name):
    client = OpenAI()
    prompt = f'''
    Given the related conversation of Speaker1 and Assistant: {original} as background information, could that directly implys the question that {question}?
    Your answer should be aligned with the personal traits of Speaker1 and Assistant.
    Your answer should also recall the previous conversation between Speaker1 and Assistant.
    Your answer shoud only be 'YES' or 'No'.
    '''
...
def main(
        ...
        summy_info = 'summarized_intent_facts.json',
        output_response_file = 'intent_full_response.json',
        output_retrieve_file = 'intent_retrieve_text.json'
    ):
    ...
    for index, uni_fact in tqdm(enumerate(sum_fact[len(final_response):200]), desc='Processing conversations'):
        speaker1_fact = []
        ...
        summarizer.build_hierarchy(speaker1_fact)
        ...
        for qa_pair in questions[index]:
            all_response_dic = {'ground_truth': ground_truth_prompt(qa_pair['question'], qa_pair['target_conv'], model_type)}
            all_response_dic['question'] = qa_pair['question']
            all_retrevial_dic = {'question': qa_pair['question']}
            all_retrevial_dic['ground_truth'] = qa_pair['target_conv']
