...
class HierarchicalClusteringSummarizer:
    ...
    def summarized_topic_prompt(self, text):
        ...
        prompt = f'''
        Please summarize {text} in one sentence to only contain the main information? 
    ...
    def select_fact_batch(self, question, facts):
        ...
        Could that possibly affect the answer of the question: {question}?
...
def main(
    ...
    for index, uni_fact in tqdm(enumerate(sum_fact), desc='Processing conversations'):
        speaker1_fact = []
        ...
        for qa_pair in questions[index]:
            ...
            all_retrevial_dic['all_fact'] = hierarchy["level_0"]["summaries"]
