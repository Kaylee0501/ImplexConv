...
def get_top_conversation_list_ultrachat(
    conv_list,
    model,
    conversation_embeddings,
    conversations,
    ...
    for conv in tqdm(conv_list):
        ...
        for conversation_emb, conversation, index in zip(
            ...
            similarity = util.cos_sim(conv_embedding, conversation_emb).mean().item()
            top_similarity_scores.append((similarity, conversation, index))


        sorted_conversations = sorted(top_similarity_scores, key=lambda x: x[0], reverse=True)
        
        top_k = []
        top_k_score = []
        for i in range(k):
            top_score, top_dialogue, _ = sorted_conversations[i]
            formatted_text = top_dialogue
            formatted_dialogues = formatted_text
            tmp = "\n".join(formatted_dialogues)
            top_k.append(tmp)
            top_k_score.append(top_score)
    ...
    for conv, conv_score, inquiry_emb in tqdm(
        ...
        inquiry_similarities_per_conv = []
        for conv_emb, score_emb in zip(conv_embedding, conv_score):
            similarity = util.cos_sim(inquiry_emb, conv_emb).mean().item()
            inquiry_similarities_per_conv.append(similarity < score_emb)
        inquiry_similarities.append(inquiry_similarities_per_conv)
    return top_conversation, top_conversation_score, inquiry_similarities


def generate_conversation_dataset_ultrachat(
    model_name="BAAI/bge-large-en-v1.5",
    input_file="../full_synthetic_conversation_cc_redial_1.json",
    output_file="full_synthetic_conversation_cc_redial_ultrachat_1.json",
    embedding_file="/home/rdharmani/persona/LT-implicit-memory/generate_data/search_datasets/ultrachat_conversation_embeddings.pkl"
):
    ...
    inquiries_embeddings = model.encode(inquiries, convert_to_tensor=True, device="cuda")
    ...
    if os.path.exists(embedding_file):
        ...
        count_multiturn = 0
        # Otherwise, process dataset to compute and store embeddings
        for i in tqdm(range(len(ds["train_gen"]))):
            # Process only multi-turn conversations
            if len(ds["train_gen"][i]["messages"]) > 1:
                count_multiturn = count_multiturn + 1
                # Collect text content of each turn
                conversation = [x["content"] for x in ds["train_gen"][i]["messages"]]
                conversations.append(conversation)
    ...
    print("Retrieving persona conversations...")
    persona_conversation, persona_conversation_score, inquiry_similarities = get_top_conversation_list_ultrachat(
        conv_list=personas,
        model=model,
        conversation_embeddings=conversation_embeddings,
        conversations=conversations,
    ...
    for i, (pc, pc_score, inquiry_bool) in enumerate(zip(persona_conversation, persona_conversation_score, inquiry_similarities)):
        data[i].update({
            "ultrachat_trait_conv": pc,
            "ultrachat_trait_conv_score": pc_score,
            "ultrachat_inquiry_bool": inquiry_bool
        })
...
generate_conversation_dataset_ultrachat()
